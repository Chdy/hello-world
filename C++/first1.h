...
public:							//数据类型的成员变量在后续章节(traits编程技巧)介绍
  ...
  //下面是成员函数
  allocator() {}//默认构造函数, 在 stl_config.h中定义,要么为空,要么为 throw()异常机制
  allocator(const allocator&)   {}	//复制构造函数
  template <class _Tp1> allocator(const allocator<_Tp1>&)   {}//泛化的复制构造函数
  ~allocator() {}//析构函数
 ...
 //这里的 alloc 在 SGI STL 中默认使用的是__default_alloc_template<__NODE_ALLOCATOR_THREADS, 0>这个实现（见第402行）
  ...
  size_type max_size() const   //返回可成功配置的最大值
